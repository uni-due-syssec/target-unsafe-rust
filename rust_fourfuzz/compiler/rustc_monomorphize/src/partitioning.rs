...
use rustc_middle::mir::{BasicBlock, Body, ClearCrossCrate::Set, Safety::*, Terminator, TerminatorKind::Call, TerminatorKind::InlineAsm};
use rustc_hir::Unsafety::Unsafe;
use std::fs::OpenOptions;

fn bb_is_unsafe<'tcx>(tcx: TyCtxt<'tcx>, body: &Body<'tcx>, bb_index: BasicBlock) -> bool {
    let len_stmnts: usize = body.basic_blocks[bb_index].statements.len();

    let mut stmt_index = 0;
    while stmt_index < len_stmnts {
        if let Set(data) = &body.source_scopes[body.basic_blocks[bb_index].statements[stmt_index].source_info.scope]
            .local_data
        {
            if let ExplicitUnsafe(_id) = data.safety {
                return true;
            };
        };
        stmt_index += 1;
    ...
    let terminator: &Terminator<'_> = &body.basic_blocks[bb_index].terminator();
    match terminator.kind {
        Call { ref func, .. } => {
            let func_ty = func.ty(body, tcx);
            let sig = func_ty.fn_sig(tcx);
            if let Unsafe = sig.unsafety() {
                return true;
            }
        }
        InlineAsm { .. } => {
        ...
        _ => {
            if let Set(data) = &body.source_scopes[terminator.source_info.scope]
                .local_data
            {
                if let ExplicitUnsafe(_id) = data.safety {
                    return true;
...
fn contains_unsafe_code<'tcx>(tcx: TyCtxt<'tcx>, body: &Body<'tcx>) -> bool {
    let mut visited_bb: Vec<usize> = Vec::new();
    let bb_len = body.basic_blocks.len();
    for i in 0..bb_len {
        let basic_block = BasicBlock::from_usize(i);
        if !visited_bb.contains(&i) && !body.basic_blocks[basic_block].is_cleanup {
            visited_bb.push(i);
            if bb_is_unsafe(tcx, body, basic_block) {
                return true;
            }
        }
    }
...
fn collect_and_partition_mono_items(tcx: TyCtxt<'_>, (): ()) -> (&DefIdSet, &[CodegenUnit<'_>]) {
    ...
    use std::env;
    if let Ok(filename) = env::var("RUST_UNSAFE_LIST") {
        let crates = match env::var("RUST_UNSAFE_CRATES") {
            Ok(crates_list)  => crates_list.split(",").map(| s| s.to_string()).collect(),
            Err(_) => Vec::new()
        };

        let crate_name = tcx.crate_name(LOCAL_CRATE).to_string();
        
        if crates.len() == 0 || crates.contains(&crate_name){

            let all_fns : Vec<&ty::Instance<'_>> = items.iter().filter_map(| mono_item | {
                match *mono_item {
                    MonoItem::Fn(ref instance) => Some(instance),
                    _ => None
                }
            }).collect();

            let unsafe_fns : Vec<&&ty::Instance<'_>> = all_fns.iter().filter(| func | {
                let body = tcx.instance_mir(func.def);
                contains_unsafe_code(tcx, body)
            }).collect();

            let syms : Vec<&str> = unsafe_fns.iter().map(|func|{
                tcx.symbol_name(***func).name
            }).collect();

            let all_names = syms.join("\n");

            let output = format!("#BEGIN unsafe in {crate_name}\n{all_names}\n#END unsafe in {crate_name}");
            {
                ...
                writeln!(file, "{}", output).expect("Could not write to file");
            }
        }
    }
