...
static std::list<std::string> denylist_functions;
static bool init_denylist = false;

void read_denylist(){

    char *denylist = getenv("AFLRS_LLVM_DENYLIST");
    
    if (denylist == NULL){
        return;
    }
    
    std::ifstream denylist_stream;
    denylist_stream.open(denylist);
    std::string line;

    if (!denylist_stream){
        report_fatal_error("Unable to open AFL_LLVM_DENYLIST");
    }

    getline(denylist_stream, line);

    while (denylist_stream) {
        ...
        if (line.find(":") != std::string::npos) {
            report_fatal_error("invalid entry in AFLRS_LLVM_DENYLIST: %s", org_line.c_str());
        }

        if (line.length() > 0) {
            denylist_functions.push_back(line);
        }
...
bool ModuleSanitizerCoverage::InjectCoverage(Function &F,
    ...
    bool is_on_denylist = false;

    if (!denylist_functions.empty()) {
        std::string instFunction = F.getName().str();
        
        for (std::list<std::string>::iterator it = denylist_functions.begin();
          it != denylist_functions.end(); ++it) {
